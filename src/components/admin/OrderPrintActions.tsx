...
export function OrderPrintActions({
  ...
  function printViaHiddenFrame(content: string, documentTitle: string) {
    ...
    iframe.onload = () => {
      ...
      if (!frameWindow) {
        setPrintError("Unable to render the print preview. Please try again.");
        cleanup();
        return;
      }
    ...
    frameDocument.open();
    frameDocument.write(printableDocument(content, documentTitle));
    frameDocument.close();
  }
  ...
  async function print(selection: PrintSelection) {
    ...
    printViaHiddenFrame(content, `${restaurant.name} order ${orderReference(order.id)}`);

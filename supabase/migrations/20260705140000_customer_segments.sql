...
--
-- One tenant-scoped, read-only round-trip that powers /admin/customers:
--   { summary, matched, contactable_matched, pagination, items }
--
-- Table-native segments (New/Repeat/VIP/Inactive/consent/high-spender) come
-- straight from the denormalized, Completed-only customers columns
-- (total_orders/total_spend/last_order_at) — no order history touched.
-- Order-derived segments (preferred fulfilment, morning/midnight, item buyers)
-- use ONE grouped pass over this tenant's completed orders (no N+1).
--
-- Thresholds mirror src/lib/customer-insights.ts (SEGMENT_RULES) so the summary
-- counts and the per-card badge agree. SECURITY DEFINER + service_role-only
-- execute: it bypasses RLS and must only be called from server-side
-- service-role code (the /admin/customers server component), never a browser JWT.

-- Supporting index for the per-phone completed-order aggregates.
...
-- Per-phone facts from this tenant's completed orders, computed in one pass.
order_facts as (
  select
    o.customer_phone,
...
-- Preferred fulfilment per phone: most common completed fulfilment,
-- tie-broken by the most recent completed order (mirrors getCustomerInsights).
pref as (
...
-- Lock execution to service-role only (matches get_customer_context grants).
